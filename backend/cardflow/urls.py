...
urlpatterns = [
    ...
    path(
        "api/feedback/",
        FeedbackAndRatingViewSet.as_view({"get": "list", "post": "create"}),
        name="feedback",
    ),
    path(
        "api/feedback/user/<int:pk>/",
        FeedbackAndRatingViewSet.as_view({"get": "retrieve"}),
    ),
    path(
        "api/bestseller/",
        BestSellerCardListView.as_view(),
        name="bestseller",
    ),
]
